#!/bin/env perl
# extract sequence and info of off-target insertions
use strict;
use warnings;
...
while(my $line = <IN>) {
	...
	while($cigar =~ /(\d+)([MIDNSHPX=])/g) {
		...
		elsif($op eq 'I') { # insert only
			if(!($chr eq $rname && $start - $max_dist <= $insert_start && $insert_start + 1 <= $end + $max_dist) && $len >= $min_insert) { # current insert location is off target region
